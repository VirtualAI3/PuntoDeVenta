...
class Usuarios extends BaseController
{
    ...
    protected $reglas, $reglasLogin, $reglasCambio,$reglasEditar;
    ...
    public function __construct()
    {
        ...
        $this->reglas = [
            'usuario' => [
                ...
                'errors' => [
                    'required' => 'El campo {field} es obligatorio.',
                    'is_unique' => 'El usuario debe ser unico'
                ]
            ],
            'password' => [
                'rules' => 'required',
    ...
    {   
        ...
        $datos=$this->logs->obtenerNombre();
        

        $data=['titulo'=>'Logs de acceso','datos'=>$datos];
        echo view('header');
        echo view('usuarios/acciones',$data);
        echo view('footer');
    }
    public function editar($id, $valid = null)
    {
        ...
        if ($valid != null) {
            $data = ['titulo' => 'Editar unidad', 'datos' => $unidad, 'validation' => $valid, 'cajas' => $cajas, 'roles' => $roles];
        } else {
            $data = ['titulo' => 'Editar unidad', 'datos' => $unidad, 'cajas' => $cajas, 'roles' => $roles];
        }
    ...
    public function actualizar()
    {
        if ($this->request->getMethod() == "post" && $this->validate($this->reglasEditar)) {
            $this->usuarios->update($this->request->getPost('id'), ['nombre' => $this->request->getPost('nombre'), 'id_caja' => $this->request->getPost('id_caja'),'id_rol'=>$this->request->getPost('id_rol')]);
            return redirect()->to(base_url() . '/usuarios');
        } else {
