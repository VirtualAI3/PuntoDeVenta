...
class TemporalCompra extends BaseController
{
    ...
    public function insertar_venta($id_producto,$cantidad,$id_venta)
    {
        $error='';

        $producto=$this->productos->where('id',$id_producto)->first();
        if($producto){
            $datosExiste=$this->temporal_compra->porIdProductoCompra($id_producto,$id_venta);

            if($datosExiste){
                $cantidad=$datosExiste->cantidad+$cantidad;
                $subtotal=$cantidad*$datosExiste->precio;
                $this->temporal_compra->actualizarProductoCompra($id_producto,$id_venta,$cantidad,$subtotal);
            } else {
